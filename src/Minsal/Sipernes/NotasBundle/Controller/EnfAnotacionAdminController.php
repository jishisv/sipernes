...
class EnfAnotacionAdminController extends CRUDController
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                $fechaingreso = $object->getFechaIngresoAnota()->format('Y-m-d H:i:s');
                
                $id = $object->getId();
                //$diag = $object->getIdCtlDiag()->getId();
                $empcor = $object->getIdEmpCorr()->getId();
                $exp = $object->getIdExpediente()->getId();
                $sql_query = "select count(date_trunc('minute', fecha_ingreso_anota)) as total from enf_anotacion where  id_emp_corr=$empcor and id_expediente=$exp and
 date_trunc('minute', fecha_ingreso_anota) = date_trunc('minute', TIMESTAMP '$fechaingreso')";
...
//                tema1 = $session->get('clap');
                ...
                if ($existe['total'] > 1) {
                    ...
                    //$sql_querybitacora = "delete from enf_bitacora_nota where fecha_ingreso_hist_notas = '$fechaingreso'";
                   // $consultabit = $conn->query($sql_querybitacora);
                  //  $sql_querytraza= "delete from enf_traza_nota where fecha_ingreso_tra_nota = '$fechaingreso'";
