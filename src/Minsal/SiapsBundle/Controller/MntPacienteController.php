        ...
        if ($numfilas > 0) {
            if ($establecimiento->getIdTipoEstablecimiento()->getId() == 1) {
                foreach ($query->fetchAll() as $aux) {
                    $espacio = '<a href=""> </a>';
                     if($aux['id_sexo']==2){
                    $espacio .= '<a href="' . $this->generateUrl('admin_minsal_shcp_shcpexpmaterno_create', array('paciente' => $aux['id'])) . '" class="btn btn-info">
    <span class="glyphicon glyphicon-plus-sign"></span> CLAP
                     </a>';
                    if (strcmp($tipo_busqueda, 'l') == 0) {
                        $numero = $aux['numero'];
                        $id = $aux['id'];
