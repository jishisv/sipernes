        ...
        case $1 in
                -c)
                        if [ -r "$2" ]; then
                                ...
                                ${ECHO} "No se puede leer el archivo de configuracion \"$2\"" 1>&2
                                exit 1
                        fi
                        ;;
                *)
...
echo "Hacer copia de seguridad en el directorio $FINAL_BACKUP_DIR"
 
if ! mkdir -p $FINAL_BACKUP_DIR; then
	echo "No se puede crear la copia de seguridad en el directorio $FINAL_BACKUP_DIR. Corregirlo!" 1>&2
	exit 1;
fi;
...
echo -e "\nTodas las copias de seguridad de la base de datos se completo!"
